...
export class SqlAnalyticsService {
  ...
  private buildDeterministicSql(
    ...
    if (/which.*highest.*battery|has.*highest.*battery/.test(q)) {
      return {
        kind: 'highest_battery',
        sql: `SELECT name, battery_mah, price_pkr, url FROM phones_analytics WHERE battery_mah IS NOT NULL ORDER BY battery_mah DESC, price_pkr ASC LIMIT 1;`,
      };
    }
